...
{
  uint8_t non_strict_order    :1;
  uint8_t non_protected_frame :1;
  uint8_t no_more_data        :1;
  uint8_t power_management    :1;
  uint8_t not_retransmission  :1;
  uint8_t last_or_unframented :1;
  uint8_t not_an_exit         :1;
  uint8_t not_to_distr_sys    :1;
} __attribute__ ((__packed__));

struct machdr
{
  uint8_t version                   :2; /* 0 for 802.11 WiFi */
  uint8_t type                      :2;
  uint8_t subtype                   :4;
...
#define DRC_AUTH_NO_MORE_VALID 0x2
#define DRC_LEAVING            0x3
#define DRC_INACTIVITY         0x4
